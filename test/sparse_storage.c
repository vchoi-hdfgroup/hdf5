 ...
 */
static herr_t
verify_get_struct_chunk_info(hid_t did, hid_t sid, hsize_t chk_index, const hsize_t *exp_offset,
                             H5D_struct_chunk_info_t exp_chunk_info[], hsize_t exp_chk_size)
{
 ...
 */
static herr_t
verify_get_struct_chunk_info_by_coord(hid_t did, hsize_t *offset, H5D_struct_chunk_info_t exp_chunk_info[],
                                      hsize_t exp_chk_size)
{
    hsize_t                 out_chk_size = 0; /* Size of an allocated/written chunk */
    haddr_t                 out_addr     = 0; /* Address of an allocated/written chunk */
    H5D_struct_chunk_info_t out_chunk_info[50];
...
test_sparse_filter(hid_t fapl)
{
    ...
    /* TBD: verify nfilters is correct */
...
test_dense_chunk_api_on_sparse(hid_t fapl)
{
    ...
    H5E_BEGIN_TRY
    {
        status = H5Dwrite_chunk(did, H5P_DEFAULT, 0, offset, CHK_SIZE, direct_buf);
    }
    H5E_END_TRY
    /* TBD: should fail */
    ...
    H5E_BEGIN_TRY
    {
        status = H5Dget_chunk_info(did, H5S_ALL, 0, NULL, NULL, &addr, NULL);
    }
    H5E_END_TRY
    /* TBD: should fail */
    ...
    H5E_BEGIN_TRY
    {
        status = H5Dget_chunk_info_by_coord(did, offset, NULL, &addr, NULL);
    }
    H5E_END_TRY
    /* TBD: should fail */
    ...
    H5E_BEGIN_TRY
    {
        ...
        udata.last_index = -1;
        status           = H5Dchunk_iter(did, H5P_DEFAULT, &iter_cb, &udata);
    }
    H5E_END_TRY
    /* TBD: should fail */
